...
namespace fms::gamma {
	...
	inline double put(double f, double sigma, double k, double t)
	{
		...
		std::pair<double, double> pairs = convert(s);
		double a = pairs.first;
		double b = a;
		double first_part = k * gamma::cdf(k/f , a, b);
		double second_part = f * gamma::cdf(k / f, a + 1, b);

		//calculate put value
		double put = first_part - second_part;
